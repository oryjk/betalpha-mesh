...
| `betalpha-active-fund-star-rating` | 问某只主动权益基金的星级评估、综合评估结果 |
| `betalpha-etf-comprehensive-score` | 问某只权益型ETF的综合评估、星级 |
| `betalpha-dividend-style-rotation` | 问核心红利/类债红利/周期红利的买卖信号、红利组合持仓 |

后三个为**投研场景技能**：内容由 Betalpha Mesh 场景目录（中台生成、人工审阅后发布）
的 Skill 原样导出，随发布管道同步；Mesh 侧场景更新后重新发布本技能集即可拿到新版。
其余场景可经 MCP `search_scenario_skills` 搜索、`explain_scenario_skill` 获取结构化
执行计划在线使用。
...
方式导入（要求根目录有 SKILL.md），导入后它会按问题类型把调用路由到上述
子技能。
...
- 0.2.0：新增投研场景技能 betalpha-active-fund-star-rating、
  betalpha-etf-comprehensive-score、betalpha-dividend-style-rotation
